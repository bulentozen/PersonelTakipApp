...
namespace PTWebMVC.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        public async Task<ActionResult> EditUser(UsersViewModel model)
        {
            ...
            {
                var yeniRoleId = roles.First(x=>x.Id==model.RoleId).Name;
                userManager.AddToRole(model.UserId,"");
                var eskiRoleId = roles.First(x=>x.Id==user.Roles.ToList().First().RoleId).Name;
                userManager.RemoveFromRole(model.UserId,eskiRoleId);
            }
